!> @brief This module contains subroutines for parsing command-line arguments and input files.
!> @details The functionality in this module is designed to provide a flexible way to control
!>          the program from the command line and a structured input file. However, in the
!>          current version of FQparam, most of these features are not used, and the main
!>          program relies on hard-coded values and the `read_input_data` subroutine from
!>          the `input_module`.
...
contains
  ...
  !> @details This subroutine is intended to parse command-line flags like -h (help),
  !>          -i (input file), -o (output file), and -t (trajectory file).
  !> @param[in] onlyinput A logical flag to control which arguments are processed.
  !> @note This is currently NOT USED in the main program.
  ...
  !> @brief Reads parameters from a structured input file.
  !> @details This subroutine is designed to parse a keyword-based input file to set up
  !>          a simulation (likely for a molecular dynamics program, given the keywords
  !>          like CUTOFF, NUMIT, COORD, etc.).
  !> @note This is currently NOT USED in the FQparam optimizer program.
  ...
  !> @brief Checks for incompatible or problematic input parameters.
  !> @details This subroutine is intended to validate the parameters read from the input file,
  !>          checking for things like atoms being outside the simulation box or questionable
  !>          choices for simulation parameters (e.g., a very large timestep).
  !> @note This is currently NOT USED in the FQparam optimizer program.
